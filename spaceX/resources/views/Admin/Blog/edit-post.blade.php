            ...
            <div class="row">
              <div class="col-lg-9 col-md-12">
                ...
                <div class="card card-small mb-3 ">
                  <div class="card-body">
                  <form action="/edit-post-{{$post->id}}" class="add-new-post" method="post">
                  @csrf
                      <input class="form-control form-control-lg mb-3" type="text" value="{{$post->title}}" name="title" placeholder="Your Post Title">
                   

                   
                     
                      <textarea name="content" name="content" class="form-control my-editor" rows="16">{!!$post->content!!}</textarea>
                      ...
                      <input id="thumbnail" style="border-radius:0px;"  value="{{$post->cover_images}}"class="form-control" type="text"  name="cover_images" >
                  </div>
                  <img id="holder" src='{{ asset("$post->cover_images") }}' style="margin-top:15px;  max-height:200px">
                </div>
                </div>
                      ...
                      <li class="list-group-item px-3 pb-2">
                        ...
                        @foreach ($Category as $item)
                          ...
                          <div class="custom-control custom-checkbox mb-1">
                              <input type="checkbox" 
                              @foreach ($CategoryPost as $sortChecked)
                               {{ old('id',$item->id) == $sortChecked->blog_category  ? 'checked' : '' }}
                               @endforeach
                               class="custom-control-input" value="{{$item->id}}" name="blog_category[]" id="category{{$item->id}}" >
